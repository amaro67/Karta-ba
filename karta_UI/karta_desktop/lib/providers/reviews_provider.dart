...
class ReviewsProvider extends ChangeNotifier {
  ...
  void updateAuthProvider(AuthProvider authProvider) {
    // Auth provider is passed in constructor
  }
  ...
  Future<void> loadEventReviews(String eventId, {int page = 1, int pageSize = 10}) async {
    ...
    try {
      _eventReviews = await ApiClient.getEventReviews(eventId, page: page, pageSize: pageSize);
      _error = null;
    } catch (e) {
      _error = e.toString();
  ...
  Future<void> loadUserReview(String eventId) async {
    ...
    try {
      _userReview = await ApiClient.getUserReviewForEvent(_token!, eventId);
    } catch (e) {
      _userReview = null;
  ...
  Future<bool> createReview(String eventId, CreateReviewRequest request) async {
    ...
    try {
      final review = await ApiClient.createReview(_token!, eventId, request);
      _userReview = review;
      _canReview = false;
  ...
  Future<bool> updateReview(String reviewId, UpdateReviewRequest request) async {
    ...
    try {
      final review = await ApiClient.updateReview(_token!, reviewId, request);
      _userReview = review;
