...
class ApiClient {
  ...
  static Future<Map<String, dynamic>> post(String endpoint, Map<String, dynamic> data, {String? token}) async {
    try {
      print('🔵 ApiClient.post: $baseUrl$apiPrefix$endpoint');
      print('🔵 ApiClient.post: Request body: ${jsonEncode(data)}');
      print('🔵 ApiClient.post: Token present: ${token != null}');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          ...
          print('🔴 ApiClient.post: Error parsing error response: $e');
          errorMessage = 'Request failed (Status: ${response.statusCode}): ${response.body.isNotEmpty ? response.body.substring(0, response.body.length > 200 ? 200 : response.body.length) : "Empty response"}';
        }
        throw Exception(errorMessage);
      }
    } catch (e) {
      print('🔴 ApiClient.post: Exception: $e');
      if (e is SocketException) {
        throw Exception('Unable to connect to server. Please check your connection.');
      }
      rethrow;
